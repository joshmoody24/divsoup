...
const ALL_HTML_ELEMENTS = [
  ...
  "dialog", "dir", "div", "dl", "dt", "em", "embed", "fencedframe", "fieldset", "figcaption",
  "figure", "font", "footer", "form", "frame", "frameset", "h1", "head", "header", "hgroup",
  "hr", "html", "i", "iframe", "img", "input", "ins", "kbd", "label", "legend", "li", "link",
  "main", "map", "mark", "marquee", "menu", "meta", "meter", "nav", "nobr", "noembed",
  ...
  "search", "section", "select", "selectedcontent", "slot", "small", "source", "span", "strike",
...
const DEPRECATED_ELEMENTS = new Set([
  "acronym", "big", "center", "dir", "font", "frame", "frameset", "marquee", "nobr", "noembed",
  "noframes", "param", "plaintext", "rb", "rtc", "strike", "tt", "xmp",
]);

const VOID_ELEMENTS = ["area", "base", "br", "col", "embed", "hr", "img", "input", "link", "meta", "source", "track", "wbr"] as const;
...
export const evaluateLegacyRule = (id: AchievementId, context: AchievementContext): boolean => {
  const { doc, rawHtml } = context;

  switch (id) {
    case "anarchic_style_sheets.main": return doc.querySelectorAll("body style").length >= 3;
    case "ascii_art.main": return hasAsciiArtComment(rawHtml);
    case "backwards_compatibility.main": return /<!--\s*\[\s*if\s+IE\s*\].*?<!\[endif\]\s*-->/is.test(rawHtml);
    case "bigheaded.main": return doc.querySelectorAll("head *").length >= 25;
    ...
    case "bob_ross.main": return doc.querySelector("canvas, picture") !== null;
    ...
    case "class_warfare.main":
      return Array.from(doc.querySelectorAll("*[class]"))
        .some((el) => ((el.getAttribute("class") ?? "").trim().split(/\s+/).filter(Boolean).length > 50));
    case "classy.bronze": return getClassRatio(doc, rawHtml) > 0.1;
    case "classy.silver": return getClassRatio(doc, rawHtml) > 0.25;
    case "classy.gold": return getClassRatio(doc, rawHtml) > 0.333;
    case "classy.platinum": return getClassRatio(doc, rawHtml) > 0.5;
    ...
    case "div_soup.bronze": return getDivRatio(doc) > 0.25;
    case "div_soup.silver": return getDivRatio(doc) > 0.5;
    case "div_soup.gold": return getDivRatio(doc) > 0.75;
    case "div_soup.platinum": return getDivRatio(doc) > 0.9;
    case "dynamic_content.main": return doc.querySelector("output") !== null;
    case "empty_calories.main":
      return Array.from(doc.querySelectorAll("div, span")).filter((el) => el.children.length === 0 && (el.textContent ?? "").trim() === "").length >= 10;
    case "flashbang.gold": {
    ...
    case "hyperlink_collector.bronze": return countUniqueExternalDomains(doc) >= 5;
    case "hyperlink_collector.silver": return countUniqueExternalDomains(doc) >= 10;
    case "hyperlink_collector.gold": return countUniqueExternalDomains(doc) >= 25;
    case "hyperlink_collector.platinum": return countUniqueExternalDomains(doc) >= 50;
    case "impa.main":
      return /\.attachShadow\s*\(|<template\s+[^>]*shadowroot\s*=\s*["'](?:open|closed)["'][^>]*>/i.test(rawHtml);
    case "important_person.main": return (rawHtml.match(/!important/g) ?? []).length >= 10;
    case "locality_of_appearance.main":
      return attrContentLength(doc, "style") > attrContentLength(doc, "class");
    case "lorem_ipsum.main": return (doc.body?.textContent ?? "").toLowerCase().includes("lorem ipsum");
    case "master_of_elements.bronze": return countValidElementsUsed(doc) >= 17;
    case "master_of_elements.silver": return countValidElementsUsed(doc) >= 60;
    case "master_of_elements.gold": return countValidElementsUsed(doc) >= 118;
    case "master_of_elements.platinum": return missingHtmlElements(doc).size === 0;
    case "millionth_visitor.main": return doc.querySelector("blink, marquee") !== null;
    case "ok_boomer.main":
    ...
    case "phd_purist.main":
      return Array.from(doc.querySelectorAll("math")).some((math) =>
        math.children.length >= 5 || math.querySelector("mfrac, msqrt, mroot, msubsup, munderover, mtable") !== null,
      );
    ...
    case "progressive.main": return doc.querySelector("progress") !== null && doc.querySelector("meter") !== null;
    case "quirky.main": return !/<!DOCTYPE html>/i.test(rawHtml);
    case "regressive_enhancement.main":
      return Array.from(doc.querySelectorAll("noscript")).some((el) => (el.textContent ?? "").replace(/\s/g, "").length < 100);
    ...
    case "scriptonite.platinum": {
      ...
      if (body.children.length !== 1 || body.firstElementChild?.tagName.toLowerCase() !== "pre") return false;
      const pre = body.firstElementChild;
      if (!pre) return false;
      const preAttrsOk = Array.from(pre.attributes).every((attr) => attr.name === "style");
      const headOk = Array.from(head.children).every((child) => child.tagName.toLowerCase() === "meta");
      return preAttrsOk && headOk && html.tagName.toLowerCase() === "html";
    }
    case "self_love.main": return doc.querySelector('a[href="#"]') !== null;
    case "semantic_snob.gold": return ["header", "nav", "main", "article", "section", "aside", "footer"].every((tag) => doc.querySelector(tag) !== null);
    case "semantic_snob.platinum":
      return ["header", "nav", "main", "article", "section", "aside", "footer"].every((tag) => doc.querySelector(tag) !== null) && doc.querySelector("div, span") === null;
    case "slot_machine.main": return doc.querySelector("template slot + slot + slot") !== null;
    case "small_data.main": return doc.querySelector('script[type="application/ld+json"], [itemscope], [itemtype], [itemprop]') !== null;
    case "soap_box.main":
      return Array.from(rawHtml.matchAll(/<!--([\s\S]*?)-->/g)).some((match) => match[1].trim().split(/\s+/).filter(Boolean).length > 100);
    case "test_in_prod.main": return /console\.log\s*\(/.test(rawHtml);
    case "todo.bronze": return rawHtml.toUpperCase().includes("TODO");
    case "todo.silver": return rawHtml.toLowerCase().split("todo").length >= 3;
    case "todo.gold": return rawHtml.toLowerCase().split("todo").length >= 12;
    case "too_meta.main": return doc.querySelectorAll("head meta").length >= 8;
    case "tower_of_babel.main": return new Set(Array.from(doc.querySelectorAll("*[lang]")).map((el) => el.getAttribute("lang") ?? "").filter(Boolean)).size >= 2;
    ...
    case "type_hints.natural_language_static_typing": return doc.querySelector('input[spellcheck="true"], textarea[spellcheck="true"]') !== null;
    case "type_hints.type_hints": return doc.querySelector("datalist") !== null;
    case "vintage.main": return doc.querySelector("table table") !== null;
    ...
    case "we_do_things_a_little_different.main":
      return Array.from(doc.querySelectorAll("span")).some((span) => span.querySelector("div") !== null);
    case "web_1_0_certified.main":
      return /<!DOCTYPE\s+HTML\s+PUBLIC\s+"-\/\/W3C\/\/DTD\s+HTML\s+3\.2(\s+Final)?\/\/EN"/i.test(rawHtml);
    case "you_are_amazing_embed.main": return doc.querySelector("object, embed, iframe") !== null;
    case "youtube_junkie.main": {
      const iframeCount = doc.querySelectorAll("iframe[src*='youtube.com'], iframe[src*='youtu.be']").length;
      const objectCount = doc.querySelectorAll("object[data*='youtube.com'], object[data*='youtu.be']").length;
      const embedCount = doc.querySelectorAll("embed[src*='youtube.com'], embed[src*='youtu.be']").length;
    ...
    case "zalgo.main": return /[^\p{M}][\p{M}]{3,}/u.test(rawHtml);
...
const countValidElementsUsed = (doc: Document): number => {
  ...
  const used = new Set(
    Array.from(doc.querySelectorAll("*")).map((el) => el.tagName.toLowerCase()).filter((tag) => valid.has(tag as (typeof ALL_HTML_ELEMENTS)[number])),
  );
  return used.size;
};
...
const attrContentLength = (doc: Document, attribute: string): number =>
  Array.from(doc.querySelectorAll(`*[${attribute}]`)).map((el) => el.getAttribute(attribute) ?? "").join(" ").length;

const hasOnAttribute = (doc: Document): boolean =>
  Array.from(doc.querySelectorAll("*")).some((el) => Array.from(el.attributes).some((attr) => attr.name.startsWith("on")));
...
const detectWebFrameworks = (doc: Document): string[] => {
  ...
  if (doc.querySelector("[data-reactroot], [data-reactid], script[src*='react']")) frameworks.push("React");
  if (doc.querySelector("[ng-app], [ng-controller], [ng-version], script[src*='angular']")) frameworks.push("Angular");
  if (doc.querySelector("[v-bind], [v-model], [v-for], [v-if], [data-v-]")) frameworks.push("Vue");
  if (doc.querySelector("[data-ember-view], .ember-application, script[src*='ember']")) frameworks.push("Ember");
  if (doc.querySelector("[data-svelte-component], [data-svelte-hydrate], script[src*='svelte']")) frameworks.push("Svelte");
...
const analyzeVoidElements = (rawHtml: string): { withSlash: number; withoutSlash: number; total: number } => {
  let withSlash = 0;
  let withoutSlash = 0;
...
const extractFromMediaQuery = (css: string, scheme: "dark" | "light"): string | null => {
  const regex = new RegExp(`@media\\s*\\(\\s*prefers-color-scheme\\s*:\\s*${scheme}\\s*\\)[^{]*{[^}]*(background(-color)?:\\s*([^;]*))`, "i");
...
const extractGeneralBackground = (htmlStyle: string, bodyStyle: string, styleContent: string): string | null => {
...
const hasMeaningfulColorSchemeQuery = (doc: Document, scheme: "dark" | "light"): boolean => {
  const styleBlocks = Array.from(doc.querySelectorAll("head style")).map((style) => style.textContent ?? "");
  const pattern = new RegExp(`@media\\s*\\(\\s*prefers-color-scheme\\s*:\\s*${scheme}\\s*\\)\\s*\\{[^}]*(html|body|main|\\.container|#container|#root|#app|\\.app|#main|\\.main|\\.content|#content)[^}]*\\}`, "i");
  return styleBlocks.some((style) => pattern.test(style));
};

const extractBackgroundColor = (doc: Document, rawHtml: string, colorScheme?: "dark" | "light"): string | null => {
...
const isLikelyCodeComment = (comment: string): boolean => {
  ...
  const definitiveMarkers = ["[if", "function", "var ", "class=", "<script", "</script>", "<![endif]"];
